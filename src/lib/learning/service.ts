import { getEffectiveClock, isValidDateOnly } from './date-kst';
import { createExtraBatch as createExtraBatchTransition, getOrCreateReviewFlow as getOrCreateReviewFlowTransition, type QueueRegistryIndex } from './queue';
...
export class LearningService {
  ...
  completeEpisode(command: CompleteEpisodeCommand): MutationResult {
    ...
    return this.run(command, 'episode_completed', (state, today) => completeEpisodeTransition(state, command, today),
      (stored) => Boolean(stored.episodes[command.episodeId]) && command.expressionIds.every((id) => Boolean(stored.expressions[id])));
  }
  ...
  markUnsure(command: MarkUnsureCommand): MutationResult {
    ...
    return this.run(command, 'unsure_accelerated', (state, today) => markUnsureTransition(state, command, today),
      (stored) => Boolean(stored.expressions[command.expressionId]));
  }
  ...
  rateReview(command: RateReviewCommand): MutationResult {
    ...
    return this.run(command, type, (state, today) => rateReviewTransition(state, command, today),
      (stored) => Boolean(stored.expressions[command.expressionId]));
  }
  ...
  reactivate(command: ReactivateCommand): MutationResult {
    ...
    return this.run(command, 'expression_reactivated', (state, today) => reactivateTransition(state, command, today),
      (stored) => stored.expressions[command.expressionId]?.graduated === false && stored.expressions[command.expressionId]?.reviewStage === 'R1');
  }
  ...
  answerSeasonReview(command: SeasonReviewCommand): MutationResult {
    ...
    return this.run(command, 'season_review_answered', (state, today) => answerSeasonReviewTransition(state, command, today),
      (stored) => stored.seasons[command.seasonId]?.review.responses[command.expressionId]?.eventId === command.eventId);
  }
  ...
  completeSeason(command: CompleteSeasonCommand): MutationResult {
    ...
    return this.run(command, 'season_completed', (state) => completeSeasonTransition(state, command),
      (stored) => stored.seasons[command.seasonId]?.completed === true);
  }
  ...
  private run(
    ...
    verify: (state: LearningStateV2) => boolean,
  ): MutationResult {
    if (!command.eventId || command.eventId.length > 160 || !Number.isFinite(Date.parse(command.now))) return { ok: false, code: 'invalid-command' };
    ...
    const updated = this.adapter.update((current) => {
      ...
      return result.state;
    }, { eventId: command.eventId, payloadHash: hash, verify });
